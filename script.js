...
// Weather mock — deterministic-ish "today" snapshot used across the app.
function getWeatherSnapshot(){
  return { temp: 29, desc:"Berawan sebagian", humidity: 78, rainChance: 55, wind: 8, icon:"⛅" };
}
...
function needsWaterToday(plant){
  // Simplified rule: needs water unless it rained recently (mocked) and phase is late maturation.
  const phase = getCurrentPhase(plant);
  if(phase.name === "Pematangan" || phase.name === "Panen") return Math.random() > 0.6;
  return true;
}
...
function generateRecommendations(plant){
  ...
  if(phase.name === "Vegetatif"){
    recs.push({ title:"Pemupukan Nitrogen", body:`Berikan pupuk Urea/N sekitar 5–8 gram per tanaman untuk mempercepat pertumbuhan daun dan batang pada fase ${phase.name}.` });
  }
  if(phase.name === "Pembungaan"){
    recs.push({ title:"Ganti ke Pupuk Fosfor", body:"Kurangi pupuk Nitrogen, tambahkan SP-36 untuk mendorong pembentukan bunga secara serempak." });
  }
  if(phase.name === "Generatif" || phase.name === "Pembuahan"){
    recs.push({ title:"Pemupukan Kalium (K)", body:"Tambahkan KCl untuk meningkatkan kualitas dan ketahanan buah terhadap kerontokan." });
  }
  ...
  if(weather.rainChance > 50){
    recs.push({ title:"Kurangi Frekuensi Penyiraman", body:`Curah hujan diprediksi ${weather.rainChance}% minggu ini — kurangi penyiraman manual untuk mencegah genangan.` });
  } else {
    recs.push({ title:"Pertahankan Jadwal Penyiraman", body:"Cuaca cenderung kering, pastikan penyiraman tetap konsisten sesuai kebutuhan fase." });
  }
  if(weather.humidity > 75){
    recs.push({ title:"Waspada Kelembapan Tinggi", body:"Kelembapan tinggi meningkatkan risiko jamur — periksa daun bagian bawah secara rutin." });
  }
...
function seedDataIfEmpty(){
  ...
  if(!AppState.settings){
    AppState.settings = { name:"Petani", location:"Jawa Tengah", dark:false };
    saveState("settings");
  }
}
...
function renderWeather(){
  ...
  document.getElementById('weather-hum').textContent = `${w.humidity}%`;
  document.getElementById('weather-rain').textContent = `${w.rainChance}%`;
  document.getElementById('weather-wind').textContent = `${w.wind} km/j`;
}
